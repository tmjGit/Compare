...
import tmj.model.FSCompareObject;
import tmj.model.RecursionType;
import tmj.model.RunningStatus;
...
public class FileCompareController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		Rows.createRows(fsco,2, new Pane[] {anchorPane1,anchorPane2,anchorPane3,anchorPane1});
	}
...
//			default: undefined error exception
	...
	private void dropped(DragEvent event, TextField field,int compareIndex) {
        ...
        if (db.hasFiles()) {
     	   ...
     	   if(ff.size()>2)
     		   Logger.warn("Too many files dropped! We take the first two, only.");
	...
	@FXML public void langCbAction(ActionEvent event) {
		...
		ComboBox<Language> cb=(ComboBox<Language>) event.getSource();
		LanguageSet.setLanguage(cb.getValue().getName());
	}
