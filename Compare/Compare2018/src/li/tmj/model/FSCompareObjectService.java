...
public class FSCompareObjectService extends Service<Boolean> {
	...
	private HashMap<String,FSItem> comparedItems=new HashMap<>();
	...
	public HashMap<String, FSItem> getComparedItems() {
	...
	private boolean compare(boolean compareFull) {//compareInternRecursiveNoThread(boolean compareFull) { 
		...
		FSItem item=new FSItem(files[0], matches); // Is this enough? matches is an array, so it stores the refertence and should have the actualized values at any time.
		comparedItems.put( files[0].getAbsolutePath(), item );
	...
	private boolean compareIntern(FSCompareCriterion c) {
...
//		}
	...
	private boolean compareSingleNoThread(File[] files,FSCompareCriterion c,boolean updateValue,boolean concrete)  { 
		boolean b=compareSingleCheck(files,c,updateValue,concrete);
		File f=files[0];
		String name=f.getName();
		String path=f.getPath();
		path=path.substring(0, path.length()-name.length());
		FSItem item=new FSItem(name,path,f.isDirectory() ? "dir" : "file",new boolean[] {b});//TODO asynchrones "füllen" der Ergebnisse der criteria1
		addFSItem(item);//TODO public Zugang zur observable list? oder binding?
		return b;
	}
			
	private boolean compareSingleCheck(File[] files,FSCompareCriterion c,boolean updateValue,boolean concrete)  { //comparer all criteria
	...
	public void addFSItem(FSItem item) {
